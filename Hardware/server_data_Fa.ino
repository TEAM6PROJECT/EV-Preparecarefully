...
// 서버 IP 및 포트 설정
IPAddress server(3, 39, 234, 31);  // 서버 IP 주소
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    Serial.println("1 : Send car BAT TMEP + GPS LOC");
    ...
    if (value == 1) {
      CanData_cellvolt();
      data_p();
      Serial.println("action 1 : Send car BAT TMEP_HIGH + GPS LOC");
      sendDataToServer_ChargeState();  //chargeState에 GPS 정보가 포함되어있음
      sendDataToServer_MT();
    } else if (value == 2) {
      CanData_cellvolt();
      data_p();
