...
void loop() {
  ...
  if (Serial.available()) {  //시리얼 모니터를 통해 차량의 상황을 시뮬레이션
    ...
    Serial.println("1 : Situation BAT TMEP_HIGH + GPS LOC");
    Serial.println("2 : Situation Cell balance critical");
    Serial.println("3 : Situation ChargeState_ON");
    Serial.println("4 : Situation ChargeState OFF");
    ...
    if (value == 1) {
      ...
      Serial.println("2 : Situation Cell balance critical");  // 셀 밸런스가 틀어진 상황을 가정함 (1~2개 셀의 volt가 엇나간 경우)
      cell_1 = 124;
      cell_2 = 122;

      Serial.println("Update complete");
    } else if (value == 3) {
      Serial.println("3 : Situation ChargeState_ON");  // 충전중인 상황을 가정함
      charge_state = 128;

      Serial.println("Update complete");

    } else if (value == 4) {
      Serial.println("4 : Situation ChargeState OFF");  // 충전중에 충전기가 분리된 상황을 가정함
      charge_state = 0;
  ...
  if (mcp2515.readMessage(&read_can_msg) == MCP2515::ERROR_OK) {
    ...
    if ((22, HEX) == ((int)read_can_msg.data[2], HEX)) {  // 22인경우는 확장 PID, 배터리 관련 데이터 전달
      ...
      if (a == 3) {  //03는 셀별 전압
        ...
        write_can_msg.data[4] = cell_1;  //n	cell42
        write_can_msg.data[5] = 154;     //o cell43
        write_can_msg.data[6] = 153;     //p cell44
        write_can_msg.data[7] = cell_2;  //q cell45
      ...
      if (a == 4) {  //03는 셀별 전압
        ...
        write_can_msg.data[3] = 155;  //ah cell94
        write_can_msg.data[4] = 154;  //ai cell95
        write_can_msg.data[5] = 153;  //aj cell96
        write_can_msg.data[6] = 255;  //ak
        write_can_msg.data[7] = 170;  //al
      ...
      if (a == 5) {  //03는 셀별 전압
        ...
        //이후 바이트들은 그냥 데이터
        write_can_msg.data[1] = 10;            //d
        write_can_msg.data[2] = 20;            //e -> e/2 해서 % 로 나누면 현재 배터리 잔량 % State_of_Charge_BMS
        write_can_msg.data[3] = 30;            //f
        write_can_msg.data[4] = 2;             //g
        write_can_msg.data[5] = 2;             //h
        write_can_msg.data[6] = 1;             // i
